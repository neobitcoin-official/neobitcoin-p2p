# Messages
The bitcoin protocol specifies a set of [messages](https://en.bitcoin.it/wiki/Protocol_specification) that can be sent from peer to peer. `neobitcoin-p2p` provides support for some of these messages.
...
As well as sending "tx" and "block" messages with neobitcoin instances:
...
## List of Messages
### Version
The version message (`ver`) is used on connection creation, to advertise the type of node. The remote node will respond with its version, and no communication is possible until both peers have exchanged their versions. By default, neobitcoin advertises itself as named `neobitcoin` with the current version of the `neobitcoin-p2p` package.
